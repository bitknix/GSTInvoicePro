  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      ...
      try {
        ...
        // For foreign customers, ensure GSTIN is set to URP if it's null/empty
        if (data.customer) {
          if (!data.customer.gstin || data.customer.gstin === 'N/A') {
            if ((data.customer.country && data.customer.country !== 'India') || 
               (data.supply_type && ['Export with Tax', 'Export without Tax', 'SEZ with Tax', 'SEZ without Tax'].includes(data.supply_type))) {
              data.customer.gstin = 'URP';
            }
          }
        }
